...
class NodeRoutesTest extends NodeTestBase {
  ...
  public function testRevisionRoutes() {
    ...
    $this->assertSession()->pageTextContainsOnce(sprintf('A page with node: (%s), langcode: (%s), and without revision', $nid, $langcode));

    $this->drupalGet("node/$nid/revisions/$current_rid/view");
    $this->assertSession()->pageTextContainsOnce(sprintf('A page with node: (%s), langcode: (%s), and node revision: (%s)', $nid, $langcode, $current_rid));

    $this->drupalGet("node/$nid/revisions/$initial_rid/revert");
    $this->assertSession()->pageTextContainsOnce(sprintf('A page with node: (%s), langcode: (%s), and node revision: (%s)', $nid, $langcode, $initial_rid));

    $this->drupalGet("node/$nid/revisions/$initial_rid/revert/it");
    $this->assertSession()->pageTextContainsOnce(sprintf('A page with node: (%s), langcode: (%s), and node revision: (%s)', $nid, 'it', $initial_rid));
